...
import { FoodType, Food } from '@/utils/types'
import Typo from '@/components/common/typo'
import * as ImagePicker from 'expo-image-picker'
import { BlurView } from 'expo-blur'

interface EditFoodFormContentsProps {
  control: Control<FieldValues>
  food: Food | null
}

const EditFoodFormContents = ({ control, food }: EditFoodFormContentsProps) => {
  const [image, setImage] = useState<string | null>(food?.image || null)

  useEffect(() => {
    if (food?.image) {
      setImage(food.image)
      control._formValues.image = food.image
    }
  }, [food])
    ...
    <View className="gap-6">
      ...
      <View>
        ...
        <Controller
          control={control}
          name="price"
          defaultValue={food?.price?.toString() || ''}
      ...
      <View>
        ...
        <Controller
          control={control}
          name="quantity"
          defaultValue={food?.quantity?.toString() || ''}
...
export default EditFoodFormContents
