...
const OrderDetails = ({ items, food }: OrderDetailsProps) => {
  const auth = useAuth()
  const filteredItems = items.filter(item => auth.user?.uid === item.storeOwnerId)
    ...
    <BlurView intensity={20} className="rounded-3xl overflow-hidden mb-4">
      <View className="bg-white/90 p-4">
        <View className="flex-row items-center mb-4">
          ...
          <Typo className="text-gray-800 font-semibold text-lg">Order Details</Typo>
        </View>
        <View className="bg-gray-50 rounded-2xl p-4">
          ...
          <ImageWrapper
            source={
              food.image ? { uri: `data:image/jpeg;base64,${food.image}` } : CANTEEN_IMAGE
            }
